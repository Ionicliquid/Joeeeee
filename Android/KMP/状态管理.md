...
```
1. 在当前快照内执行业务逻辑，以SnapshotMutableStateImpl为例读写状态都是基于currentSnapshot，makeCurrent将当前线程快照设置为指定快照；
# UI 组织方式
    ...
    if (inserting) {  
        ...
        if (reader.hadNext) {  
            ...
            if (changeListWriter.pastParent) {  
                ...
                // directly update the parent.                changeListWriter.updateValue(value, groupSlotIndex)  
            }  
        } else {  
...
# 总结
1. SnapshotMutableState 是 SnapShot 与 MutableState 的组合，作为状态的管理中的被观察者，对当前 SnapShot 范围内发生的读写进行监听；
2. SnapShot 快照，分为全局快照和临时快照，临时快照是全局快照的副本，每次组合创建
	1. 组合期间设置读写监听，用来标识下次状态变化的重组范围；
	2. 组合期间的状态的修改需要主动 apply应用到全局快照；
3. Recomposer负责监听状态的变化和发起重组的流程；
	1. setContent时创建并启动，设置全局状态监听，状态变化时，标脏指数据，恢复 UI线程关联协程的执行；
	2. 等待 Choreographer.FrameCallback回调后开启重组